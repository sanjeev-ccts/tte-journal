/* roster-bridge.js — v9 (adds live sub-label update on dashboard) */
(function () {
    'use strict';
    console.log('[bridge] v9 loaded');
    ...
    function getIdentity() {
        ...
        if (!raw) return null;
        try { const o = JSON.parse(raw); if (o && o.team) return o; } catch (e) {}
        return null;
    }
    ...
    /* Issue 3 — live update the dashboard roster card sub-label */
    ...
    function pushLeg(legs, bundle, ds, trainNo, isArrival) {
        if (!trainNo) return;
        ...
        if (isArrival) {
            arrDate = ds;
            depDate = info.arrNextDay ? addDays(ds, -1) : ds;
        } else {
            depDate = ds;
            arrDate = info.arrNextDay ? addDays(ds, 1) : ds;
        }
        legs.push({
            id: 'pygs_' + depDate + '_' + trainNo + (isArrival ? '_arr' : '_dep'),
            train: trainNo,
            depDate: depDate, depTime: info.dep,
    ...
    function buildProjectedLegsForYear(year) {
        ...
        while (d <= end) {
            ...
            if (st.kind === 'sameDay') {
                pushLeg(legs, bundle, ds, st.out, false);
                pushLeg(legs, bundle, ds, st.in, true);
            } else if (st.kind === 'departureOnly') {
                pushLeg(legs, bundle, ds, st.out, false);
            } else if (st.kind === 'arrivalOnly') {
                pushLeg(legs, bundle, ds, st.in, true);
            }
    ...
    function installProjectedLegs() {
        window.buildProjectedDutyLegsForYear = buildProjectedLegsForYear;
        window.buildPygsProjectedLegsForYear = buildProjectedLegsForYear;
    }
        ...
        window.openQuickAddCalendar = function () {
            ...
            if (!bundle) {
                ...
                window.firebaseGet(window.firebaseRef(window.db, 'roster/' + hq())).then(function (snap) {
                    const dd = snap.val();
                    if (!dd || !dd.teamMaster || !dd.linkProgramme) {
                        window.showModal('Roster Empty', 'Open Duty Roster first to set up your roster.', 'warning', false);
                        return;
                    }
                    dd._hq = hq();
                    window.rosterData = dd;
                    try { localStorage.setItem('roster_cache_hq_' + hq(), JSON.stringify(dd)); } catch (e) {}
                    openNow();
    ...
    for (let i = 0; i < total; i++) {
        ...
        const st = window.RosterCore.getDayStatus(bundle, id.team, id.member, ds);
        cells.push({ dayNum: dn, dateStr: ds, status: st, isToday: ds === todayStr });
    }
    ...
    for (let r = 0; r < rows; r++) {
        ...
        while (col < 7) {
            ...
            if (!c) { html += '<div class="qadd-day qadd-empty"></div>'; col++; continue; }
            const st = c.status;
            const ts = c.isToday ? ' style="outline:2px solid var(--blue);outline-offset:1px;"' : '';

            const nxt = col < 6 ? rowCells[col + 1] : null;
            const isPair = st.kind === 'departureOnly' && nxt && nxt.status && nxt.status.kind === 'arrivalOnly';

            if (isPair) {
                const todayCell = c.isToday || nxt.isToday;
                const wrapStyle = todayCell ? ' style="grid-column:span 2; outline:2px solid var(--blue); outline-offset:1px;"' : ' style="grid-column:span 2;"';
                html += '<div class="qadd-trip-wrapper"' + wrapStyle + '>';
                html += '<div class="qadd-subday qadd-out" onclick="window._pygsQaddTap(\'' + c.dateStr + '\')"><div class="qadd-daynum">' + c.dayNum + '</div><span class="qadd-tag">OUT</span><span class="qadd-trainno">' + st.out + '</span></div>';
                html += '<div class="qadd-subday qadd-in" onclick="window._pygsQaddTap(\'' + nxt.dateStr + '\')"><div class="qadd-daynum">' + nxt.dayNum + '</div><span class="qadd-tag">IN</span><span class="qadd-trainno">' + nxt.status.in + '</span></div>';
                html += '</div>';
                col += 2;
                continue;
            }
            ...
            if (st.kind === 'blank') {
                html += '<div class="qadd-day qadd-empty"></div>';
            } else if (st.kind === 'rest') {
                html += '<div class="qadd-day qadd-rest" onclick="window._pygsQaddTap(\'' + c.dateStr + '\')"' + ts + '><div class="qadd-daynum">' + c.dayNum + '</div><span class="qadd-tag">REST</span></div>';
            } else if (st.kind === 'crossing') {
                html += '<div class="qadd-day qadd-crossing" onclick="window._pygsQaddTap(\'' + c.dateStr + '\')"' + ts + '><div class="qadd-daynum">' + c.dayNum + '</div><span class="qadd-tag">CROSSING</span></div>';
            } else if (st.kind === 'sameDay') {
                html += '<div class="qadd-day qadd-outin" onclick="window._pygsQaddTap(\'' + c.dateStr + '\')"' + ts + '><div class="qadd-daynum">' + c.dayNum + '</div><span class="qadd-tag">OUT</span><span class="qadd-trainno">' + st.out + '</span><span class="qadd-tag">IN</span><span class="qadd-trainno">' + st.in + '</span></div>';
            } else if (st.kind === 'departureOnly') {
                html += '<div class="qadd-day qadd-out" onclick="window._pygsQaddTap(\'' + c.dateStr + '\')"' + ts + '><div class="qadd-daynum">' + c.dayNum + '</div><span class="qadd-tag">OUT</span><span class="qadd-trainno">' + st.out + '</span></div>';
            } else if (st.kind === 'arrivalOnly') {
                html += '<div class="qadd-day qadd-in" onclick="window._pygsQaddTap(\'' + c.dateStr + '\')"' + ts + '><div class="qadd-daynum">' + c.dayNum + '</div><span class="qadd-tag">IN</span><span class="qadd-trainno">' + st.in + '</span></div>';
            }
            col++;
        }
    }
    grid.innerHTML = html;
        ...
        window._pygsQaddTap = function (ds) {
            ...
            if (st.kind === 'departureOnly') prefill(ds, st.out, bundle, false);
            if (st.kind === 'arrivalOnly') prefill(ds, st.in, bundle, true);
        };

        window._pygsQaddPick = function (ds, trainNo) {
    window._backButtonClosingModalId = 'quick-add-cal-modal';
    window.closeCustomModal(true);
    const b = syncBundleFromCache();
    if (b) prefill(ds, trainNo, b, false);
};

        function prefill(ds, trainNo, bundle, isArrival) {
    const info = window.RosterCore.getTrainInfo(bundle, trainNo);
    ...
    function loop() {
        ...
        if (ticks >= MAX) {
            ...
            setTimeout(function () { updateRosterSubLabel(); mirrorTeamKey(); installAll(); }, 500);
            return;
        }
        setTimeout(loop, 100);
    }
    ...
    document.addEventListener('visibilitychange', function () {
        if (!document.hidden) { updateRosterSubLabel(); mirrorTeamKey(); installAll(); }
    });
