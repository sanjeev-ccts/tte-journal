...
function buildCalendarCardHtml(b,letter,member,year,month,navPrefix){
    const cells=buildCalendarGridCells(b,letter,member,year,month);
...
function buildCalendarGridCells(b,letter,member,year,month){
    ...
    for(let r=0;r<rows;r++){
        ...
        while(col<7){
            ...
            if(isPair){
                ...
                html+='<div class="cal-subday '+statusClass(cur)+'" onclick="window.rosterOpenDayDetail(\''+cur.dateStr+'\')">'+dayInner(cur)+'</div>';
                html+='<div class="cal-subday '+statusClass(nxt)+'" onclick="window.rosterOpenDayDetail(\''+nxt.dateStr+'\')">'+dayInner(nxt)+'</div>';
                html+='</div>';
                col+=2;
            }else if(cur){
                html+='<div class="cal-day '+statusClass(cur)+(cur.isToday?' cal-day-today':'')+'" onclick="window.rosterOpenDayDetail(\''+cur.dateStr+'\')">'+dayInner(cur)+'</div>';
                col++;
            }else{
